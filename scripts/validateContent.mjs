...
for (const required of [
  ...
  'Creative Prototype',
  'Opposite Experiment',
  'Exploring Ideas and Making Moves'
]) {
  if (!combined.includes(required)) failures.push('required Part 2 content missing: ' + required);
}
...
if (new Set(frameworkMatches).size !== frameworkMatches.length) {
  failures.push('duplicate named frameworks remain inside Part 2');
}
...
console.log('PASS Part 2 curriculum, de-duplication, conformity, public-firewall, and secret-boundary validation');

...
record('Part 2 welcome renders', await page.getByRole('heading', { name: 'Exploring Ideas and Making Moves' }).isVisible());

await page.getByTestId('begin-part2').click();
...
await page.getByTestId('final-reflection').fill(
  'My biggest Part 2 insight is that useful movement comes from small experiments that create evidence, reveal assumptions, and make the next decision more informed.'
);
...
const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('ctj.part2.state.v2') || '{}'));
...
record('Mobile welcome renders', await mobilePage.getByRole('heading', { name: 'Exploring Ideas and Making Moves' }).isVisible());
